...
* **LiteSpeed Cache:** under Cache → Excludes, add your export directory (default
  `/wp-content/uploads/wp-mfa-exports/`) to "Do Not Cache URIs" and `output_format`
  to "Do Not Cache Query Strings". The `Accept` header route also needs a rewrite
  rule in `.htaccess`; the Caching section of the README on GitHub has the full
  configuration.
