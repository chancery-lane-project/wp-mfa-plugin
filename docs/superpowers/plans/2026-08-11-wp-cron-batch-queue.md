...
- **BLOCKING BEFORE MERGE — `watchdog()` does not reset `schedule_failures` on a successful reschedule (Task 11 review, Important).** `maybe_reschedule()` resets the counter only when *it* schedules successfully; `watchdog()` calls `wp_schedule_single_event()` directly and writes nothing back. So a job rescued three times over three hours by the hourly watchdog — each rescue proving scheduling works — still accumulates to `MAX_SCHEDULE_FAILURES` and self-fails with "WP-Cron may be disabled or blocked", which is both wrong and the opposite of what the counter is meant to measure (three *consecutive* failures). Fix: on a successful schedule, `watchdog()` resets `schedule_failures` to `0` and saves with the record's own `lock_token`, mirroring `maybe_reschedule()`. Add the missing test — nothing currently asserts the counter's state after a watchdog rescue.
- **BLOCKING BEFORE MERGE — the `admin_init` nudge blocks a page load for the full cron budget (Task 11 review, Important).** `maybe_nudge()` calls `run_tick()`, which spends up to 30s (or whatever `max_execution_time` yields). Invisible for cron; for an admin who merely loaded a page it reads as a broken site. The nudge's job is to get a dead chain moving, not to make maximum progress in that request. Fix: give the nudge its own short cap (a few seconds) — thread an optional budget override through `run_tick()`/`process()` — and pass a context argument to the `markdown_for_agents_tick_budget` filter (`'cron'` vs `'nudge'`) so a site owner can tune the two independently. Document the second filter argument in Task 17.
- **BEFORE MERGE — `public_job_record()` should be an allowlist, not `unset( 'lock_token' )` (Task 13 review, Important).** `lock_token` is a bearer credential: anyone holding it can call `GenerationJob::save()` and write to the job record. A denylist fails *open* — the next field added to the record's internal shape is exposed by default unless someone remembers to update this one method, and the only test (`assertArrayNotHasKey('lock_token', …)`) would not catch it. Switch to explicit construction or `array_intersect_key()` against a named list of public keys, so a newly-added internal field is hidden until someone deliberately exposes it.
- **BEFORE MERGE — no test exercises `scope` sanitisation (Task 13 review, Important).** The retired handler had `test_post_type_is_sanitised`; nothing replaced it. Both current scope tests pass already-clean strings through a mocked `StageFactory`, so neither exercises what `sanitize_text_field( wp_unslash( … ) )` does to a messy value. Not a security gap — `scope` only needs to survive as plain text and is allowlisted downstream by `StageFactory` — but a real coverage regression. Add a test capturing the value that reaches `build_stage_list()` from a deliberately messy `$_POST['scope']`.
- **Task 17 acceptance addition (Task 13 review, Minor):** because `Admin`'s two queue collaborators are nullable-with-a-500 rather than required, a Task 14 wiring mistake stays silent until someone clicks Start. Task 17's manual pass must therefore exercise the Start button specifically, not just the cron chain.
- **Post-merge suggestion, deliberately NOT done (Task 13 review, Minor):** extracting the two job endpoints plus `public_job_record()` into their own class (e.g. `Jobs\JobAjaxController`), following `Admin`'s existing `SettingsPage`/`MetaBox` composition pattern, would let its collaborators be *required* rather than optional and resolve the silent-mis-wiring trade for free. Not taken now: it is a refactor of already-reviewed working code late in the plan, and `Admin` did not grow in Task 13 (433 → 416 lines, same method count).
- **Minor, same review:** the three bundle-rebuild touchpoints (`BundleGenerator::mark_stale_and_schedule()`, `BundleStage::process_batch()`, `JobRunner::after_completion()`) are each well documented but none cross-references the other two; add a one-line "see also" to each. Also — reviewed and explicitly **not** renamed: `mark_stale_and_schedule()` still describes what it does, and its unconditional-delete comment explains the split.
