...
8. **AMENDED 2026-07-06 (Felix sign-off): the bundle is a `.zip`, not `.tar.gz`.** The first live build failed — PharData's tar writer is ustar-only, capping filenames at 100 characters, and real TCLP clause slugs exceed that (108 chars observed). OKF §3 permits zip explicitly; PharData zip has no name limit; entries are deflated via `compressFiles(Phar::GZ)`; the media type becomes `application/okf-bundle+zip` (equally unregistered). Shelling out to system tar was rejected (no-shell-dependency constraint); skipping long-named files was rejected (silently incomplete bundle). References to `.tar.gz`/`+gzip` elsewhere in this plan predate the amendment.
