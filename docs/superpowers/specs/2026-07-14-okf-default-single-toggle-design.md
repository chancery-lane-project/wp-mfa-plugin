...
- `Commands::generate_manifest()` becomes a thin wrapper calling `$this->generator->write_manifests( $export_base, $types )`, preserving the existing `--with-manifest`/`--incremental` CLI behavior exactly (§ unaffected, see below). Remove the now-unused `use Tclp\WpMarkdownForAgents\Generator\ManifestGenerator;` import at `Commands.php:12` once the construction logic moves to `Generator`.
- Three call sites now call `write_manifests()` immediately before `BundleGenerator::build()`, each already having access to a `Generator` instance and the enabled post types via `ExportPolicy::enabled_post_types( $options )`:
  ...
  - The debounced cron rebuild: `on_rebuild_bundle()` lives directly on `BundleGenerator` (`BundleGenerator.php:217`) and is hooked as the `markdown_for_agents_rebuild_bundle` cron callback in `Plugin.php:152` (`$this->loader->add_action( 'markdown_for_agents_rebuild_bundle', $bundle_generator, 'on_rebuild_bundle' )`). `BundleGenerator`'s constructor stays untouched (see below), so the hook itself is repointed instead: `Plugin.php:152` changes from registering `$bundle_generator`'s method directly to registering a closure — defined in the same `define_generator()` method where both `$generator` (the local `Generator` instance, `Plugin.php:106`) and `$bundle_generator` (`Plugin.php:145`) are already in scope — that calls `$generator->write_manifests( Options::get_export_base( $options ), ExportPolicy::enabled_post_types( $options ) )` and then `$bundle_generator->on_rebuild_bundle()`. `Loader::add_action()` (`src/Core/Loader.php:30`) takes an object plus a string method name (`array( $component, $callback )`), not a bare callable — pass the closure as `$component` and `'__invoke'` as `$callback`, relying on `Closure::__invoke()`.
...
**`BundleGenerator`'s constructor is unchanged** — no new dependency is injected into it, so the three existing `new BundleGenerator( $options )` sites (`src/Core/Plugin.php:145`, `tests/Unit/Discovery/ArdCatalogTest.php:35,61`, `tests/Unit/Generator/BundleGeneratorTest.php:46`) require no updates for that reason. The only wiring change is the cron hook repointing described above (`Plugin.php:152`), which doesn't touch `BundleGenerator` itself. `BundleGenerator::build()`'s existing "include `manifest.json` when present" behavior (it already includes the file if it finds one in the tree) now reliably finds one, because a caller always writes it first.

**`src/Admin/Admin.php` needs a new `use` import for `ExportPolicy`** (and `Options`, if not already imported) to call `ExportPolicy::enabled_post_types( $options )` in `maybe_rebuild_bundle()` — confirmed `Admin.php` has `$this->generator` and `$this->options` already available there, but does not currently import `ExportPolicy`.
