...
- **Concurrent-execution guard:** the `wp_next_scheduled()` checks above stop duplicate *pending cron events*, but not two ticks physically running at once — e.g. two admin tabs open, both hitting the `admin_init` nudge in overlapping requests, both seeing no event scheduled yet (because neither has scheduled anything), both calling the tick handler directly with the same still-valid `lock_token`. The `lock_token` write-time check doesn't stop this, since both would still hold a matching token. Each tick therefore wraps its body in a short-lived mutex: `add_option( 'markdown_for_agents_job_tick_lock', 1, '', false )` (atomic insert-fails-if-already-exists, unlike a plain `update_option`) acquires it; a tick that fails to acquire returns immediately, doing no work and scheduling nothing (the tick that *did* acquire the lock will reschedule when it finishes, so no work is lost); `delete_option()` releases it at the end of the tick, including on the early-return paths (done/failed).
...
| `tests/mocks/wordpress-mocks.php` | Extend with `wp_schedule_single_event`/`wp_next_scheduled` mock hooks; ensure the `add_option` stub fails when the key already exists (atomic-insert semantics), needed for the tick mutex |
