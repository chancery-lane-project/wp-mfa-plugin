...
class LlmsTxtGenerator {
	 ...
	 */
	public function __construct( private readonly array $options = array() ) {}
	...
	public function generate( string $export_base ): bool {
		...
		$type_dirs = glob( rtrim( $export_base, '/' ) . '/*', GLOB_ONLYDIR ) ?: array();
	...
	public function parse_frontmatter( string $filepath ): array {
		...
		while ( false !== ( $line = fgets( $handle ) ) ) {
			...
			if ( $in_fm ) {
				...
				if ( '' !== $trimmed && ( ltrim( $line ) !== $line || '-' === $trimmed[0] ) ) {
					continue;
				}
